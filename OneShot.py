...
def _gradient_penalty(net_D, data, generated_data, gamma=10):
...
def ceshi(net,ii,epoch):
    ...
    fake_date=net(A.unsqueeze(0))
    fk_im = toimage(fake_date[0,0, 32, :, :].unsqueeze(0).unsqueeze(0))
    # img_test.append(fk_im)
    if (ii%500)==0:
        save_filenamet = 'fake%s_%s.bmp' % (epoch,ii)
        img_path = os.path.join('./check/test_run/', save_filenamet)
        save_image(fk_im.squeeze(0), img_path)
...
def main():
    ...
    net_g = networks.define_G(1, 64, opt.ngf, 'CTnet', opt.norm,
                      not opt.no_dropout, opt.init_type, opt.init_gain, opt.gpu_ids)
    #summary(net_g, (1,257, 257))
    init_weights(net_g)
    net_d.to(device)
    net_g.to(device)
    if load_net:
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for ii, data in enumerate(dataset):
            ...
            loss=criterion(fake_date,real_data)
            loss.backward()
            optimizer_g.step()
    ...
    #             img_test=[]
                 ...
                 save_image(fk_im.squeeze(0), img_path)
                 real_im = toimage(real_data[0, 0, 32, :, :].unsqueeze(0).unsqueeze(0))
                 save_filenamet = 'real%s.bmp' % epoch
                 img_path = os.path.join('./check/img/', save_filenamet)
                 save_image(real_im.squeeze(0), img_path)

                 ceshi(net_g,ii,epoch)
                 message = '(epoch: %d, iters: %d, loss1: %.8f) ' % (epoch, ii, loss)
                 print(message)
