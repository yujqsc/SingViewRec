...
def main():
    ...
    # 优化器
    optimizer_d = torch.optim.Adam(net_d.parameters(), lr=0.0008,betas=[0.6,0.9])
    optimizer_g = torch.optim.Adam(net_g.parameters(), lr=0.0005,betas=[0.3,0.9])
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for ii, data in enumerate(dataset):
            ...
            if ii % 10 == 0:
                ...
                img_test=[]
                img_test.append(toimage(torch.irfft(fake_date.permute(1, 2, 3, 0), 2, onesided=False)[32, :, :].unsqueeze(0)))
                img_test.append(toimage(torch.irfft(real_data.squeeze(0).permute(1, 2, 3, 0), 2, onesided=False)[32, :, :].unsqueeze(0)))
