...
def main():
    ...
    # 定义判别器与生成器的网络
    #net_d = NLayerDiscriminator(65, opt.ndf, n_layers=5)#batchnorm
    net_d =networks.define_D(65, opt.ndf, 'basic',
    ...
    # 优化器
    optimizer_d = torch.optim.Adam(net_d.parameters(), lr=0.00001,betas=[0.3,0.9])
    optimizer_g = torch.optim.Adam(net_g.parameters(), lr=0.00001,betas=[0.3,0.9])
    #optimizer_d =torch.optim.RMSprop(net_d.parameters(), lr=0.001)
    #optimizer_g =torch.optim.RMSprop(net_g.parameters(), lr=0.001)
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for ii, data in enumerate(dataset):
            ...
            if ii % 10 == 0:
                ...
                # img_test=[]
                #fk_im = toimage(torch.irfft(torch.roll(torch.roll(fake_date, -32, 2), -32, 3).permute(1, 2, 3, 0), 2,
                ...
                # rel_im= toimage(torch.irfft(torch.roll(torch.roll(real_data, -32, 2), -32, 3).permute(1, 2, 3, 0), 2,
                #                             onesided=False)[32, :, :].unsqueeze(0))
                ...
                message = '(epoch: %d, iters: %d, d_loss: %.3f, g_loss: %.3f, WDistance: %.3f) ' % (epoch, ii, D_cost.mean(), G_cost,Wasserstein_D)
                print(message)
                # for it in range(1, 3):
                #     plt.subplot(1, 2, it)
